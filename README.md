...
> 一个适用于嵌入式的轻量 UI 框架：  
> 用 `Page + Item + Layer(栈)` 表达菜单树，使用**链表**将所有page和item链在一起，用 **统一的插值光标 Cursor** 把所有页面变化、焦点与转场连成一体。
>
> ![功能简要演示](功能简要演示.gif)
...
- **感谢B站麻神（TrojanGeneric）的文件架构的分层管理思想**

---

## 以后要改进的

- 目前少量动画仍有 Delay，把所有 Delay 干掉，让动画/转场/弹窗全部变成逐帧推进、可打断、完全实时
